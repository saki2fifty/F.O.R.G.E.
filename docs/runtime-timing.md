...
`RuntimeClock` accumulates monotonic elapsed time. Each completed tick calls `RuntimeSimulation`, which runs an explicit Flecs pipeline selecting `FixedSimulation` systems and ordering their Phase/DependsOn relationships. Current stages are **Gameplay** (ABI v1 callback) then **Transforms** (final world evaluation). These immediate systems operate on the owning world; no Flecs worker threads are enabled. Effective local-pose capture follows completion. Presentation is an explicitly separate extraction stage on snapshot requests, not another gameplay tick. Flecs `progress(fixed_dt)` updates its frame/time metadata using the same explicit delta passed to the native callback. No physics or animation placeholder systems exist.
