...
| `.collision.json` Collision | Collision recipe importer | Box, Sphere, Capsule, Cylinder, Convex Hull, static Triangle Mesh and immutable nested Compound; explicit Mesh/LOD/part provenance | Bounded geometry, native scale/shape validation; no automatic render collision, decomposition or deformed Mesh baking | Exact Jolt5.6.0; `collision_asset`, `collision_pipeline`, `physics_acceptance_level` |
...
| Audio source / AudioClip | Audio recipe and existing runtime loader | Admitted WAV decode, immutable clip publication, clip metadata and source-free runtime loading | WAV only; MP3/FLAC remain disabled; playback uses Play, with no editor audition lifecycle | Pinned miniaudio; `audio_recipe`, `audio_pipeline`, `audio_tools`, `audio`, native Audio clip document captures |
