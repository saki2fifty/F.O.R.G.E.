...
`DocumentWorkspace` is an internal borrowed-adapter registry. It associates a transient editor key with a title, stable ImGui window identity, central docking policy, open/dirty queries, draw ownership, Save, history capabilities, close request and optional local inspection callback. Current adapters cover Scene, prefab source, Project Settings, Flecs Script, Texture import, Model import/placement, Shader import and Material source editing. Scene/Game rendering remains explicit because it owns render/input resources; registration is not a second renderer or simulation. Prefab source initially docks centrally; saved custom geometry remains authoritative.

`AssetEditors` maps supported asset types to open callbacks. Content double-click/context/Inspector opening uses this registry. Scenes, prefabs, Flecs Script, texture/model/shader import and standalone Material source documents have open handlers. Imported Material members open the owning Model import document. Do not register nonexistent Graph/UI designers or advertise unsupported asset editors.
