...
`DocumentWorkspace` is an internal borrowed-adapter registry. It associates a transient editor key with a title, stable ImGui window identity, central docking policy, open/dirty queries, draw ownership, Save, history capabilities, close request and optional local inspection callback. Current adapters cover Scene, prefab source, Project Settings, Flecs Script and Texture import. Scene/Game rendering remains explicit because it owns render/input resources; registration is not a second renderer or simulation. Prefab source initially docks centrally; saved custom geometry remains authoritative.

`AssetEditors` maps supported asset types to open callbacks. Content double-click/context/Inspector opening uses this registry. Scenes, prefabs, Flecs Script and texture import settings currently have document-open handlers. Do not register nonexistent Material/Graph/UI designers or advertise unsupported asset editors.
