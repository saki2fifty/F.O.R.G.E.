...
`DocumentWorkspace` is an internal borrowed-adapter registry. It associates a transient editor key with a title, stable ImGui window identity, central docking policy, open/dirty queries, draw ownership, Save, history capabilities, close request and optional local inspection callback. Current adapters cover Scene, prefab source, Project Settings, Flecs Script, Texture import/preview, generated Texture viewer, Model import/preview/placement, read-only Mesh/Material previews, Shader import, Material source editing and Audio clip import/metadata. Scene/Game rendering remains explicit because it owns render/input resources; registration is not a second renderer or simulation. Prefab source initially docks centrally; saved custom geometry remains authoritative.

`AssetEditors` maps supported asset types to open callbacks. Content double-click/context/Inspector opening uses this registry. Scenes, prefabs, Flecs Script, texture viewing/import, model/shader import and standalone Material source and Audio clip import documents have open handlers. Mesh and imported Material members open a read-only central Asset viewer; Open source import opens the owning Model import document. Do not register nonexistent Graph/UI designers or advertise unsupported asset editors.
