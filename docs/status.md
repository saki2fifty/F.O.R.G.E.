...
Adds exclusive OS-held project writer ownership and an explicitly enabled local editor connection with read-only/edit capabilities. The same shared authoring operations and undo history serve live scripts; tokens and session identities revoke on document transitions. Numbered replay receipts prevent duplicate commits, and busy UI/play/build activity blocks external edits. File/native execution remains unavailable through automation; MCP is still planned. Includes a Python example, connection controls, path/control-directory checks and process/socket/UI regression tests. Final Windows CI evidence is recorded after packaging; desktop acceptance is separate.
