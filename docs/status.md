...
Local core/API/CLI and editor suites pass, along with opt-in Clang AddressSanitizer/UndefinedBehaviorSanitizer tests and Windows-header syntax validation. Windows package validation is pending. See [Authoring API](authoring-api.md) for contracts and limits. Live-project automation, MCP transport, file writer coordination, generalized asset documents and animation remain pending.
