...
The preview applies local scale, then Euler X/Y/Z rotation, then world Position. ChildOf still organizes entities without propagating parent transforms. Supported prefab fields inherit through Flecs; preview snapshots resolve effective values without modifying authored ownership. Inspector edits create owning overrides.
