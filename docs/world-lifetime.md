...
Application scope
  ...
  EngineContext
    WorldContext
      ...
      scene-membership roots and scoped EntityId/UUID maps
...
Unsupported scene versions are rejected without changing the open scene. Unknown components and fields remain opaque JSON, including plugin payload/version fields. Component-version envelopes and plugin hydration remain deferred; scene identity migration is described in the scene-format documentation.
