...
void run_converter(const std::filesystem::path& executable, const std::filesystem::path& staging,
    ...
    if (!executable.is_absolute() || !std::filesystem::is_regular_file(executable))
        throw std::runtime_error("Packaged gltf2ozz converter is missing");
    ...
    std::wstring command = L"gltf2ozz --file=source.gltf --config_file=config.json";
    ...
    if (!CreateProcessW(executable.c_str(), command.data(), nullptr, nullptr, FALSE,
                        ...
                        &process))
        throw std::runtime_error("Cannot launch packaged animation converter");
    ...
    if (!AssignProcessToJobObject(job.value, child.value) ||
        ...
        throw std::runtime_error("Cannot constrain animation converter");
    }
    try {
        while (WaitForSingleObject(child.value, 10) == WAIT_TIMEOUT) {
            if (cancel.stop_requested() || std::chrono::steady_clock::now() > deadline ||
                ...
                throw std::runtime_error(
                    "Animation conversion cancelled or exceeded resource limits");
        }
        DWORD code = 1;
        if (!GetExitCodeProcess(child.value, &code) || code != 0)
            throw std::runtime_error("Animation conversion failed; previous assets retained");
    } catch (...) {
    ...
    if (child < 0)
        throw std::runtime_error("Cannot start animation converter");
    if (child == 0) {
    ...
    try {
        for (;;) {
            ...
            if (result < 0 && errno != EINTR)
                throw std::runtime_error("Cannot wait for animation converter");
            if (cancel.stop_requested() || std::chrono::steady_clock::now() > deadline ||
                output_exceeded(staging))
                throw std::runtime_error(
                    "Animation conversion cancelled or exceeded resource limits");
    ...
    if (cancel.stop_requested() || output_exceeded(staging))
        throw std::runtime_error("Animation conversion cancelled or exceeded output limits");
}
} // namespace forge::animation_detail
