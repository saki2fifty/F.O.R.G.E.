...
with tempfile.TemporaryDirectory(dir=Path.cwd()) as scratch:
    ...
    try:
        ...
        assert state['entities'][0]['components']['forge.local_translation']['x']==0.5
        ...
        assert session.step(0.5)['scene']['entities'][0]['components']['forge.local_translation']['x']==1
        (project/'gameplay.cpp').write_text(original.replace('seconds, 0.0f','2 * seconds, 0.0f'))
        ok,log=session.build()
        assert ok,log
        assert session.active!=old
        assert session.step(0.5)['scene']['entities'][0]['components']['forge.local_translation']['x']==2
        ...
        ok,log=session.build()
        assert not ok,log
        assert session.step(0.5)['scene']['entities'][0]['components']['forge.local_translation']['x']==3
        ...
        assert session.step(0.5)['scene']['entities'][0]['components']['forge.local_translation']['x']==3.5
        ...
        try:
            ...
            pass
        assert session.step(0.5)['scene']['entities'][0]['components']['forge.local_translation']['x']==4
    finally:
        session.close()
