...
int main(int argc, char** argv) {
    try {
        ...
        std::filesystem::path relocated;
        {
            ...
            save(path, source);
            AssetImportService service(lease, collision_import_registry(),
                                       {"linux", "none", "cpu"});
            ...
            auto ticket = request_collision(
                pool, root, std::make_shared<const AssetCatalog>(catalog), {source.asset()});
            require(pool.wait(ticket, 10s), "Published collision failed runtime preparation: " +
            ...
            }
            {
                ...
                const auto snapshot = snapshot_physics_debug_collision(original);
                const auto mesh =
                    std::async(std::launch::async, [components, snapshot] {
                        return prepare_physics_debug(components, {-2, 1, 1}, snapshot);
                    }).get();
                ...
                require(low < -.9 && high > .9,
                        "Native mirrored hull preview has wrong scale/bounds");
            }
            ...
            require(pool.wait(ticket, 10s) &&
                        pool.acquire(ticket).identity() != original.identity(),
            ...
            relocated = root.parent_path() / (source.asset().str() + "-relocated");
            std::filesystem::rename(packaged, relocated);
            asset = source.asset();
        } // Close authoring documents, importer and writer lock before moving the project.
        const auto hidden = root.string() + "-unavailable";
        std::filesystem::rename(root, hidden);
        runtime_collision(relocated, asset);
