...
Visible means an entity has an effective Position and is not a prefab template. It does not mean that the camera can currently see it. Owned components exclude values inherited from a prefab.
...
The current components are Position, Rotation, Scale, Tint, and Primitive. This window is read-only; use Inspector to change values. Animation-related metadata prepares property bindings for future tools; animation editing is not available in this build.
