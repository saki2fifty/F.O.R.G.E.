...
If saving fails because another program holds the file open, close that program and retry. The editor keeps the current scene, the original disk file, and the identity record. Migration preserves the old placement and independent parent behavior using **Spatial binding → World**. New parenting uses **Follow parent**. See [Transforms](transforms.md).
