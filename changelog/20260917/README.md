...
### Phase 6B — approved ancestry and target corrections

- Reject separate Static/Kinematic bodies whose effective spatial ancestry reaches a Dynamic body. Reuse the Phase 3 graph, including Explicit links, transitive nonphysics intermediaries and World boundaries; preserve structural hierarchy and local channels. Keep visual descendants and supported non-Dynamic ancestry.
- Preserve affected and ancestor identities in structured diagnostics, including failure responses after an unpublished replacement runtime is destroyed. Revalidate runtime component/hierarchy/prefab changes and recovery before solver use.
- Prepare queued movement in detached FIFO transform values before synchronizing the hierarchy. Reject targets that require different LocalScale or shear; preserve unrelated prefab transform ownership.
- Add regressions for the previous falling-parent divergence, direct/transitive/Explicit/World cases, visual children, component changes, prefab realization/reconciliation, valid/invalid recovery, rejected replacement, parent/child command ordering and scaled targets. Update the physics user guide and technical contract. No scene/identity/ABI1/recovery-version change; no Phase 6C. Full corrected-source validation and numbered delivery follow.
...
### Phase 6C — audio module and authoring

- User accepted Phase6B/Build43 Windows testing. Add only authorized audio integration; animation/navigation/runtime UI remain deferred.
- Pin private miniaudio0.11.25; add AudioSource/AudioListener schemas, typed nullable AudioClip references and boolean property editing through normal validation/Undo/prefab intent. Size scene reconciliation storage from the component registry rather than the former nine-type constant.
- Reuse AssetCatalog/ProjectPaths and version1 forge.assets.json for idempotent WAV registration, selection and contained resolution. Add a generated test tone; no importer/cooker or separate audio database.
- Add optional forge.audio provider, owner-thread Play/Stop, bounded decoded resource sharing, device notification handoff, listener/spatial snapshots, gameplay group Pause/Step/Resume and explicit no-device/null tests. Recovery rebuilds autoplay from frame0 without a new audio/physics persistence blob.
- Add exact-SDK audio capability and source commands while preserving ABI1. Add Inspector/prefab controls, user manual, technical contract and normal Console diagnostic forwarding. Schema-only/default headless contexts open no audio device.
...
Build260917-000045 was withheld after MSVC rejected C++20 reversed string/JSON comparisons in animation provenance checks. Compare explicitly extracted digest strings in runtime admission and candidate publication; the validation policy is unchanged. A fresh numbered clean build will verify the corrected source.
