...
## Exact SDK schema extraction (implementation in progress)

- Added explicit schema-stage authoring opt-in for reflected plain-value components, native Meta-derived export, stable keys/versions/module ownership/categories/defaults, structural digests independent of physical field order or labels, and engine-owned typed-reference/string transport adapters.
- Added matching shared-SDK `--inspect-sdk` command with runtime startup disabled and bounded copied metadata output. Native code stays in the worker/runtime; no editor DLL loading. Exact SDK fingerprint includes the new bridge and value transport implementation. Portable gameplay ABI1 is unchanged.
- Local opt-in/defaults/cross-world reconstruction/identity/invalid-admission tests pass. SDK process validation and full editor/scene/prefab consumers are still in progress; custom Add Component entries are not yet enabled.
- Shared SDK schema/lifetime/process checks pass together (4/4,0.77 s), including isolated schema-registration crash, failed registration, wrong fingerprint, pre-cancellation, deterministic retry and staging cleanup. Owner-thread admission follow-up passes (0.28 s). Strict ASan/UBSan/LSan custom schema/default/reference tests pass (0.27 s). Additional malformed-output, active cancellation and metadata-only physics-requirement regressions are under validation.
- Added a bounded fixed-command schema worker using the existing process supervisor. The background task transports copied metadata; Flecs reconstruction/acceptance stays on its owning thread. Schema-only composition provides no simulation services and does not call runtime startup. Original SDK permissions and normal runtime capability validation remain unchanged.
- The expanded isolated-worker suite passes (4/4,0.95 s): active cancellation, duplicate-key and oversized output, wrong profile, required-physics metadata inspection, registration crash/failure and clean retry. Moved TextureAsset/ShaderAsset value tags into the installed typed-reference header so SDK clients can name those references without importing asset-compiler/resource APIs.
- Expanded worker tests pass3/3 (1.95 s), including the one-second supervisor deadline. Typed-reference/schema tests pass (0.05 s), including collection EntityRef remapping that preserves opaque and foreign-scene data. Fixed new AssetRef native JSON serialization to match builtin UUID strings. Scene duplication integration remains separate pending work.
- Installed SDK validation passes (15.48 s): packaged client compilation, shared Flecs imports, relocated runtime and copied schema inspection, followed by the existing subsystem and module-lifetime checks. No Windows execution claim is inferred from the local Linux result.
- Rebuilt post-header-relocation sanitizer validation passes (0.27 s) with ASan, UBSan and leak detection. Manual and changed-source formatting checks pass. Windows follow-up remains pending; the full custom-component authoring path is still being implemented.

## Custom component scene/prefab integration and property widgets (continuing)

- Added world-local native component codecs for opted-in schemas, a reserved versioned identity envelope, unknown-field preservation, candidate validation, Scene Undo/Redo, native prefab inheritance/equal-value intent/Revert, and matching-runtime schema enforcement. Editor schema activation/migration and final SDK Play proof remain in progress.
- Connected declared EntityRef remapping to subtree and whole-scene duplication. Asset-file workers receive copied schema metadata; impact review includes per-property prefab intent and reports incompatible schemas as uninspected.
- The first integrated EntityRef regression exposed late engine-adapter registration: pinned Flecs compiles member cursor operations when the containing type is declared. Moved engine-owned reference metadata before module schema callbacks. Builtin reference projection now resolves actual registered C++ type IDs rather than relying on presentation names.
- Added shared nested-property, paged collection, bitmask and small-integer controls. New list entries remain detached until validated; text uses pinned Dear ImGui's std::string wrapper instead of truncating to4096 bytes. Added default reset and read-only missing/incompatible-schema displays.
- Actual input tests pass (15.34 s), including full-width integers, a5000-character string, nested unknown-field preservation and read-only behavior. Expanded integration and sanitizer reruns are in progress; this is not full Phase7 acceptance.
- Windows source audit1412cf1 compiled the editor and passed50/51 checks. The remaining capture stalled at the200% Model preview; corrected its readiness gate so later documents do not wait on a hidden Material preview, and added per-resource/frame diagnostics. A Windows rerun remains required.
- SDK extraction checkpoint8b42948b passed format and all four Windows/Linux core/shared-SDK profiles. No new numbered release was produced.
- Corrected integration passes7/7 local suites (14.25 s): custom schemas/components, asset-file operations, identities, structured prefabs, world lifetime and authoring API. Strict ASan/UBSan/LSan passes3/3 (3.03 s), including native string/reference lifetime, native runtime query values and incompatible-schema rejection. Rebuilt Inspector/process tests pass (13.57 s), including actual unavailable-component display. Manual and full-format checks pass.

## Custom component schema publication and explicit migration

- Added owning-thread publication of copied component schemas with staged native type generations. Existing scene bytes and Undo/Redo survive schema refresh; failed candidates retain the previous bindings, templates and values. Old registrations retire only after their instances/templates do.
- Added a shared explicit whole-component override command and Inspector header action. Equal-value intent remains owned; Revert and Undo/Redo return to native prefab inheritance. Missing-schema prefab components and inherited opaque extensions remain visible without becoming authored overrides.
- Added bounded forward-only named-field migration with declared aliases, collision/cycle rejection, retained removed/unknown fields and target-version defaults. Partial prefab intent does not acquire new top-level overrides. New fields inside dynamic collection entries require explicit migration defaults when no per-entry default exists.
- Extended the disposable SDK inspection worker to validate the current target declaration and prepare migration values without executing project code in the editor. Schema migration is not automatic on startup. Scene and prefab publication adapters are under validation; the end-user admission/migration workflow remains in progress.
- Source-backed correction: pinned nlohmann/json mixed-number equality can equate uint64 maximum with signed -1, or round integer/float comparisons. Authoring no-op, dirty-state, recovery-baseline and prefab publication comparisons now retain exact integer meaning. Schema defaults validate before any unchanged shortcut. No vendor patch or dependency change.
- Corrected the actual editor Save As route to pass admitted custom reference metadata when duplicating a scene; declared intra-scene EntityRefs remap, opaque references remain unchanged.
- Local schema-generation sanitizer checks pass2/2 (1.81 s). Custom component/migration/prefab/authoring regressions pass4/4 (6.58 s). Isolated SDK extraction/migration/process and actual Editor Play controller tests pass3/3 (2.39 s), including saved/reopened authored health and uint64 maximum read by native gameplay. Expanded document and sanitizer checks remain pending.
- Prior source checkpoint fb9f37a passed format and all four Windows/Linux core/shared-SDK profiles. Its Windows editor audit is still in progress. No new numbered package is claimed.

- Added Gameplay Code inspection and migration review controls, persistent project schema history (`forge.components.json`), type-owner reservations, and immutable per-version structure digests. History writes run inside staged schema activation; blocked writes retain the old native generation and document/history state. The file cannot be selected as a scene Save As destination.
- Added a separate editor admission fixture covering supervised worker output, failure retention, schema-version refresh without value migration, history persistence and actual100%/200% control rendering (passes0.21 s). The real shared-SDK tests above separately exercise genuine module loading and native gameplay values; the UI fixture does not substitute for that evidence.
- Actual editor/process regression passes15.35 s, including custom EntityRef Save As and exact unknown-value dirty/external-conflict checks. One-document migration/history tests pass2/2 (0.41 s); strict migration/native-value tests pass2/2 (2.50 s) before the latest history/UI additions. Updated history sanitizer checks are running.
...
### Backend-neutral implementation checkpoint

- Shared material shaders now use register-free bounded sampler arrays. Shader/compiler selection and geometry/compute/tessellation capability checks live in a narrow backend adapter. The pinned WebGPU route has a named-scalar declaration variant using Diligent's documented array suffix; no WebGPU execution is claimed.
- Added the source-backed renderer backend matrix. Logical Mesh/Material/Texture/Shader source identities stay neutral; existing DXBC Shader cooks remain explicitly Windows/D3D12 artifacts. Unsupported signed-skin geometry-shader paths reject clearly on devices without that feature.
- Texture/sampler data validation no longer imposes D3D12 numeric or compressed-volume restrictions on other backends. GPU upload checks Diligent format/dimension, anisotropy and state capabilities. An isolated Linux Vulkan adapter queries the numeric LOD-bias limit omitted by Diligent's public interface; Windows uses SDK constants only within that adapter. Unsupported borders reject before the pinned Vulkan conversion can substitute a different color. Authored sampler settings remain unchanged.
- Added an optional actual-material Vulkan test with SPIR-V compilation, 17 independent material samplers, environment/comparison sampling and pixel readback. Texture/backend tests pass7/7 (4.57s), including native bias/border admission and raw DDS/KTX compressed-volume data. The center pixel is68 as expected. The standalone register-free19+2 Windows FXC probe passes in run35659980484. Full changed-renderer WARP validation remains pending.
- Native source7d39d93 passes56/56 Windows audit tests (195.84s). Actual capture review still found an early thumbnail capture: the fixture counted cumulative renders instead of distinct current assets. It now requires five current ready entries. Audit startup clears cached screenshots/test logs so old failed captures cannot be mislabeled as current evidence. The stricter capture requires a new native run.

## Exact SDK runtime resource requests

- Added a world-scoped CPU resource service for Mesh, Material, Texture and Shader assets, using existing typed pools, immutable revisions, owner-thread adoption and strong retained leases. Failed replacements keep the prior usable revision. Catalog refresh prepares off-thread and adopts at runtime boundaries, including paused presentation.
